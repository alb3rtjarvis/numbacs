...
def flowmap_grid_2D(funcptr, t0, T, x, y, params, method="dop853", rtol=1e-6, atol=1e-8, mask=None):
    ...
    if mask is None:
        return _flowmap_grid_2D(funcptr, t0, T, x, y, params, method=method, rtol=rtol, atol=atol)
    else:
        return _flowmap_masked_grid_2D(
            funcptr, t0, T, x, y, params, mask, method=method, rtol=rtol, atol=atol
        )
...
def flowmap_aux_grid_2D(
    ...
    if mask is None:
        ...
        return _flowmap_aux_masked_grid_2D(
            funcptr,
            t0,
            T,
            x,
...
def flowmap_n_grid_2D(
    ...
    if mask is None:
        return _flowmap_n_grid_2D(
            funcptr, t0, T, x, y, params, n=n, method=method, rtol=rtol, atol=atol
        )
    else:
        return _flowmap_n_masked_grid_2D(
            funcptr, t0, T, x, y, params, mask, n=n, method=method, rtol=rtol, atol=atol
        )
...
# %% Unmasked versions
@njit(parallel=True)
def _flowmap_grid_2D(funcptr, t0, T, x, y, params, method="dop853", rtol=1e-6, atol=1e-8):
    """
    Computes the flow map at the final time of the ode defined by funcptr where funcptr is a
    pointer to a C callback created within numba using the ``@cfunc`` decorator. Flow map is
    computed over the grid defined by x,y. t0 denotes initial time and T denotes
...
@njit(parallel=True)
def _flowmap_n_grid_2D(funcptr, t0, T, x, y, params, n=50, method="dop853", rtol=1e-6, atol=1e-8):
    """
    Computes the flow map of the ode defined by funcptr where funcptr is a
    pointer to a C callback created within numba using the ``@cfunc`` decorator. Flow map is
    computed over the grid defined by x,y. t0 denotes initial time and T denotes
...
# %% Masked versions
@njit(parallel=True)
def _flowmap_masked_grid_2D(
    funcptr, t0, T, x, y, params, mask, method="dop853", rtol=1e-6, atol=1e-8
):
    """
    Computes the flow map at the final time of the ode defined by funcptr where funcptr is a
    pointer to a C callback created within numba using the ``@cfunc`` decorator. Flow map is
    computed over the grid defined by x,y. t0 denotes initial time and T denotes
    integration time. Masked version.
...
@njit(parallel=True)
def _flowmap_aux_masked_grid_2D(
    funcptr,
    t0,
    T,
    x,
    ...
    a pointer to a C callback created within numba using the ``@cfunc`` decorator. Flow map
    is computed over the aux grid defined by np.meshgrid(x,y) +-h. t0 denotes initial time
    and T denotes integration time. Masked version.
...
@njit(parallel=True)
def _flowmap_n_masked_grid_2D(
    funcptr, t0, T, x, y, params, mask, n=50, method="dop853", rtol=1e-6, atol=1e-8
):
    """
    Computes the flow map of the ode defined by funcptr where funcptr is a
    pointer to a C callback created within numba using the ``@cfunc`` decorator. Flow map is
