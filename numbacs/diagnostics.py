...
def ftle_grid_2D_edge(flowmap,T,dx,dy):
    """
    Compute 2D FTLE field from flowmap which is solution of ode over an initial grid defined by x
    and y for integration time T. Includes edges of grid.
        ...
        for j in range(ny):
            if j == 0:
               dydir = 'f'
                ...
                dydir = 'c'
            dxdx = finite_diff_2D(flowmap[:,:,0],i,j,dx,0,dxdir)
            dxdy = finite_diff_2D(flowmap[:,:,0],i,j,dy,1,dydir)
            dydx = finite_diff_2D(flowmap[:,:,1],i,j,dx,0,dxdir)
            dydy = finite_diff_2D(flowmap[:,:,1],i,j,dy,1,dydir)
